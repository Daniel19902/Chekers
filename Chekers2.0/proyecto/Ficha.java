...
public class Ficha
{
    ...
    public void shift(boolean top, boolean rigth, Ficha[][] listaDeFichas, Rectangle[][] casillas, int row , int column){
        Ficha mover = listaDeFichas[row][column];
        int moveTopOrDown = darPosicionDeMovimiento(top);
        int moveLeftOrRigth = darPosicionDeMovimiento(rigth);
        boolean verificarMovimiento = verificarMovimiento(mover.color, moveTopOrDown, moveLeftOrRigth, listaDeFichas, row, column);
        // movimiento
        if (verificarMovimiento){
            ...
            listaDeFichas[row+moveTopOrDown][column+moveLeftOrRigth].setPossion(false);
    ...
    public void jump(boolean top, boolean rigth, Ficha[][] listaDeFichas, Rectangle[][] casillas, int row , int column){
        Ficha mover = listaDeFichas[row][column];
    ...
    private int darPosicionDeMovimientoJump(boolean mover){
    ...
    private boolean verificarMovimiento(boolean color, int moveTopOrDown, int moveLeftOrRigth, Ficha[][] listaDeFichas, int row , int column){
        boolean confirmarMovimiento = true;
        if (color && moveTopOrDown == -1)confirmarMovimiento = false;
        else if (!color && moveTopOrDown == 1)confirmarMovimiento = false;
        else if (listaDeFichas[row+moveTopOrDown][column+moveLeftOrRigth] != null)confirmarMovimiento = false;
        return confirmarMovimiento;
    }
}
